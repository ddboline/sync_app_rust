...
all:
	...
	cp -a Cargo.toml src sync_app_lib sync_app_http \
		gdrive_lib templates scripts Makefile build/ && \
	cd build/ && \
...
cleanup:
	...
	rm -rf /tmp/.tmp.docker.sync_app_rust
	rm Dockerfile

package:
	docker run --cidfile $(cidfile) -v `pwd`/target:/sync_app_rust/target sync_app_rust/build_rust:ubuntu18.04 \
        /sync_app_rust/scripts/build_deb_docker.sh $(version) $(release)
	docker cp `cat $(cidfile)`:/sync_app_rust/sync-app-rust_$(version)-$(release)_amd64.deb .
	docker rm `cat $(cidfile)`
	rm $(cidfile)

package_xenial:
	docker run --cidfile $(cidfile) -v `pwd`/target:/sync_app_rust/target sync_app_rust/build_rust:ubuntu16.04 \
        /sync_app_rust/scripts/build_deb_docker.sh $(version) $(release)
	docker cp `cat $(cidfile)`:/sync_app_rust/sync-app-rust_$(version)-$(release)_amd64.deb .
	docker rm `cat $(cidfile)`
	rm $(cidfile)
...
test:
	docker run --cidfile $(cidfile) -v `pwd`/target:/sync_app_rust/target sync_app_rust/build_rust:ubuntu18.04 /bin/bash -c ". ~/.cargo/env && cargo test"

build_test:
	cp Dockerfile.test.ubuntu18.04 build/Dockerfile && \
	cd build/ && \
	docker build -t sync_app_rust/test_rust:ubuntu18.04 . && \
	cd ../ && \
	rm -rf build/
...
dev:
	docker run -it --rm -v `pwd`:/sync_app_rust rust_stable:latest /bin/bash || true
